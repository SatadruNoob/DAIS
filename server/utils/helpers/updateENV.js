...
async function validDockerizedUrl(input = "") {
  if (process.env.DAIS_RUNTIME !== "docker") return null;
  ...
  try {
    ...
    if (isPortAvailableFromDocker)
      return "Port is not running a reachable service on loopback address from inside the DAIS container. Please use host.docker.internal (for linux use 172.17.0.1), a real machine ip, or domain to connect to your service.";
  } catch (error) {
