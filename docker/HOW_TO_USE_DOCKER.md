# How to use Dockerized DAIS

Use the Dockerized version of DAIS for a much faster and complete startup of DAIS.

### Minimum Requirements

> [!TIP]
> Running DAIS on AWS/GCP/Azure?
...
\*DAIS by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*DAIS by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized DAIS!
...
> the docker container using DAIS as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/dais
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run DAIS in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/dais && \
...
$env:STORAGE_LOCATION="$HOME\Documents\dais"; `
...
Go to `http://localhost:3001` and you are now using DAIS! All your data and progress will persist between
...
- `git clone` this repo and `cd dais` to get to the root directory.
- `touch server/storage/dais.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of DAIS easier.

### Use the Midori AI Subsystem to Manage DAIS

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the DAIS docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in DAIS.
