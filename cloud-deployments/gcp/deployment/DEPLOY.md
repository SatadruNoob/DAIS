...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create dais-deployment --config gcp/deployment/gcp_deploy_dais_with_env.yaml
...
```
gcloud compute instances get-serial-port-output dais-instance 
```

ssh into the instance

```
gcloud compute ssh dais-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete dais-deployment 
```
